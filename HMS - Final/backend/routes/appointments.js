...
router.get('/patient', auth, async (req, res) => {
  try {
    ...
    if (req.user.role !== 'patient') {
      return res.status(403).json({ message: 'Access denied. Only patients can access their appointments.' });
    }
    
    console.log('Getting appointments for patient with userId:', req.user.id);
...
router.get('/', auth, async (req, res) => {
  try {
    console.log('GET /appointments request received from user:', req.user);
    ...
    console.log('Admin access verified, fetching all appointments');
